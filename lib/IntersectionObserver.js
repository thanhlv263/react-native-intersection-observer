...
class IntersectionObserver {
    ...
    handleScroll = throttle(() => {
        ...
        for (let index = 0; index < this.targets.length; index += 1) {
            ...
            if (horizontal) {
                isIntersecting =
                    contentOffsetWithLayout + (rootMargin.right || 0) - 50 >=
                        targetLayout.x &&
                        contentOffset.x - (rootMargin.left || 0) - 50  <=
                            targetLayout.x + targetLayout.width;
            }
            else {
                isIntersecting =
                    contentOffsetWithLayout + (rootMargin.bottom || 0) - targetLayout.height/2 >=
                        targetLayout.y &&
                        contentOffset.y - (rootMargin.top || 0) - targetLayout.height/2 <=
